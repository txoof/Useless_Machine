...
aaron ciuffo () gmail.
Txoof on Thingiverse

v1.34 14 December 2014
 TODO
  * solution for lid flopping open due to pusher arm moving so quickly?
  * merge in changes on circuit board from hobby local version of eagle designs
  * fix door jamb placement 
  * remove some catches

v1.33 10 December 2014
  * Added version number
  v1.32 08 December 2014
  * swapped out microswitch for a variant with two mounting holes
    - effing norway doesn't stock M2 bolts to mount the switch I had
  * adjusted switch hole size in lid due to laser kerf (too large!)
    - laser cutter/2d design favors kerf for inside shape
  * switched to larger hinge


v1.31 07 December 2014
  * cleaned up global varialbes
  * standardized methods for placing objects in 2D/2D 

v1.3 06 December 2014
  * moving from supportPunch to holePunch for making all support holes
  * 

v1.2 05 December 2014
  * Cleaned up a in the 2D placement routine 
  * finished many tasks
...
DONE
  X add catches on YZ faces to make service possible //v1.34
  X add one more row of catches //v1.34
  X move microswith up by 2-4mm //v1.34
  X enlarge raven to make it possible to pass switches through hole //v1.34
  X swap out switch on lid with smaller switch //v1.33
  X possibly move motor mount in -Y direction to better deal with door
  X decrease pilot holes on hinge
  X Need to add a door jamb (small piece width of door to keep from falling
    inward against slop in hinge
  X Flip over bottom piece (180 in y axis of 2D) so bottom can be decorated
  X double check radius on m3 nuts
  X check hinge placement - might need to move a little bit 
  X remove lid catch holes punched in middle support
  X rewrite the support punch sub; something more general 
  X fix faceA with regards to hole punch
  X add parameter to allow changing Q in holepunch - 
  X add more tabs to the bottom of the Y support
  X move circuit board to opposite end of box or rotate
  X wires from switch, battery pack can get caught in arm 
    - need better routing options other than hole in center bottom of support
    - move hole to -Y end of box rather than center
  X add ziptie holes for wire routing 
...
//generalized library to produce a measurable scale to compare to
...
module doorJamb() {
  ...
  hull(center=true) {
    translate([djX/2, djY/2])
      circle(r=cornerR, center=true, $fn=36);
    translate([-djX/2, djY/2])
      circle(r=cornerR, center=true, $fn=36);
    translate([-djX/2, -djY/2])
      circle(r=cornerR, center=true, $fn=36);
...
module bolt(boltDia=3, boltLen=10) {
  headThick=boltDia;
  headDia=boltDia*2;
  union() {
    cylinder(h=boltLen, r=boltDia/2, $fn=36);
    translate([0, 0, boltLen])
      cylinder(h=headThick, r=headDia/2, $fn=36);
...
module placeCatch3D() {
  ...
  translate([bX/2, bY/2-fingerW/2-fingerLidW/2-thick, bZ-thick*2-lidBolt])
    rotate([0, 90, 0])
    bolt(boltLen=10);
 
  translate([bX/2+25, bY/2-fingerW/2-fingerLidW/2-thick+lidBolt, bZ-thick*2])
    rotate([90, 0, 180])
    color("black")
    text("10mm", size=6);
  ...
  translate([bX/2, bY/2-fingerW/2-fingerLidW/2-thick, thick+lidBolt])
    rotate([0, 90, 0])
    bolt(boltLen=16); 

  translate([bX/2+25, bY/2-fingerW/2-fingerLidW/2-thick+lidBolt, thick+lidBolt*2])
    rotate([90, 0, 180])
    color("black")
    text("16mm", size=6);
...
module layout2D(uDivX, uDivY, uDivZ, uDivLX, uDivLY) {
  ...
  translate([c1e2x, c1e2y, 0])
    color("olive") internalSupport(uDivY, uDivZ, qZ=1.5);
...
module layout3D(uDivX, uDivY, uDivZ, uDivLX, uDivLY, alp=0.5) {
  ...
  color("olive", alpha=1)
    translate([supXpos, 0, bZ/2-D]) rotate([90, 0, 90])
      linear_extrude(height=thick, center=true) internalSupport(uDivY, uDivZ, qZ=1.5);
  ...
  color("indigo")
    ...
    switchSupport(uDivY, Q=ssRatio, holeZ=mswZpos); 
...
module placeSwitch3D() {
  ...
  translate([switchX+switchDia/2, switchY+switchDia, bZ-thick/2]) rotate([0, 0, 90])
    color("black")
    text("Flip -->", size=6);
...
module placeBattery3D() {
  translate([batY/2+supXpos+thick/2+batPosX, -bY/2+batZ/2+thick+batPosY+20, 
    batX/2+thick/2+batPosZ])
    rotate([-90, 90, 90])
    battery_pack();
  translate([batY/1+supXpos+thick/2+batPosX, -bY/2+batZ/2+thick+batPosY+20, 
    batX/2+thick/2+batPosZ])
    rotate([90, 0, 90])
    color("black")
    text("<-- Insert 4xAA Batteries", size=6);
}
