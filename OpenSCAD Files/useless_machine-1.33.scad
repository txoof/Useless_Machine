...
v1.33 10 December 2014
  * Added version number
  TODO
  * Need to add a door jamb (small piece width of door to keep from falling
    inward against slop in hinge
  X Flip over bottom piece (180 in y axis of 2D) so bottom can be decorated
  * possibly need to redo arm design - may be too curved & flips up door much
  X possibly move motor mount in -Y direction to better deal with door
  * !!OR!! possibly lengthen door flap by 10-20mm to make it heavier
  X decrease pilot holes on hinge
  * increase pilot holes on motor
  * merge in changes on circuit board from hobby local version of eagle designs
  * swap out switch on lid with smaller switch
  * add "Magic/More Magic" to lid?
...
TODO
  ...
  * Add stop to keep arm lid from falling inside the box
...
module layout2D(uDivX, uDivY, uDivZ, uDivLX, uDivLY) {
  ...
  c0e2x=c0e1x-catch*4.5-separation*3.5;
  ...
  c0e4x=(-brdBlt-4-separation)*1.5;
  c0e4y=c0e3y-separation-(brdBlt+4)*2;
  //board standoffs c0e4 
  translate([c0e4x, c0e4y, 0])
    color("slateblue") boardStandOff(r=2, c=4);

  c0e5x=0;
  c0e5y=c0e4y-separation-(brdBlt+4)*2;
  ...
  c0e6x=0;
  c0e6y=c0e5y-15-separation-doorJX/2;
  echo(c0e6y);
