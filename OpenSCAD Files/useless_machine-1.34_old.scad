...
module placeCatch3D() {
  ...
  //catches on YZ face (front/back)
  translate([bX/2-thick, bY/2-thick*3.5, fingerW/2+1.5*thick])
    rotate([90, 0, 90])
    catch3D(); 
 
  translate([bX/2-thick, -1*(bY/2-thick*3.5), fingerW/2+1.5*thick])
    rotate([-90, 0, 270])
    catch3D(); 
  ...
  translate([-1*(bX/2-thick), -1*(bY/2-thick*3.5), fingerW/2+1.5*thick])
    rotate([90, 0, 270])
    catch3D(); 
...
module placeServo2D() {
  translate([serY, -bZ/2+serZ, 0])
    futaba(bolt=false, project=true, center="shaft", scalePct=1.05);
}
